...
//			return;
		...
		try {
				...
				sqlQuery = "SELECT * FROM ECM_USER_SEARCH WHERE APPID=? AND EMPNO=? AND "
				...
				if (rs.next()) {
					...
					this.setSettingValue(DBUtil.clobToString(itemClob));
...
//					isLoaded = true;
	...
	private void insert() throws Exception
	{
		...
		try {
			...
			String getDBUSERSql = "INSERT INTO ECM_USER_SEARCH(ID, APPID, EMPNO, KEYNAME, KEYVALUE) "
			...
			itemClob.setString(1, this.settingValue);
	...
	private void update() throws Exception
	{
		...
		try {
			...
			sqlQuery = "UPDATE ECM_USER_SEARCH SET APPID=?, EMPNO=?, "
	...
	public void delete() throws Exception
	{
		...
		try {
			conn = DBUtil.getECMDBConnection();
		    sqlQuery="Delete from  ECM_USER_SEARCH where ID=?";		
			stmt = conn.prepareStatement(sqlQuery);
