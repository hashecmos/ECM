...
@Component({
    ...
    templateUrl: './esigndocument.component.html',
    styleUrls : ['./esigndocument.component.css']
})
...
export class EsignDocumentComponent implements OnInit {
  ...
  exportData() {
    ...
    if (startExport) {
     ...
     this.us.exportOrgESignItems(this.reportOrgCode,this.status, this.fromDate, this.toDate,selectedOption).subscribe(res => {
...
this.tabset.tabs[1].disabled = false;
    ...
    this.us.getOrgESignItems(this.reportOrgCode,this.status, this.fromDate, this.toDate).subscribe(data => this.esignDocumentCountReport(data));
  ...
  esignDocumentCountReport(data) {
    this.disableTab = false;
    this.unselectTab = false;
    this.selectTab = true;
    this.reportData = JSON.parse(data._body);
    this.esignDocumentCount = [];
    this.esignDocumentCount = this.reportData;
    $('#image').html('');
